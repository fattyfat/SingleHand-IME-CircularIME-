...
*/
    ...
    /**return #1:do nothing(在外圈無用地帶) #2:切換左右雙手輸入法 #3:切換英中日輸入法 other:output String #4:廣告*/
        ...
        if (touchRadius > fifthCircleRadius ) { //點擊座標半徑超過圖形，視為無效操作
            ...
            if ((700 <= radiusMultiple && radiusMultiple <= 800) && angleMultiple <= 7){
                return "#2";
            }else if (870 <= radiusMultiple && (32 <= angleMultiple && angleMultiple <= 38 && enableAD)) {
                return "#4";
            }else if ((635 <= radiusMultiple && radiusMultiple <= 729) && (30 <= angleMultiple && angleMultiple <= 35) && adEvent) {
                return "#5";
            }else if ((730 <= radiusMultiple && radiusMultiple <= 900) && (23 <= angleMultiple && angleMultiple <= 30) && adEvent) {
                return "#4";
            }
